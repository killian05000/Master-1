...
void matrix_add_coef(struct matrix *mat,int constraint,int variable, double value)
{
  int i = mat->next_coef;
  mat->coef[i] = value;
...
{
  for (int variable = 1; variable <= nbr_variables; variable++)
    matrix_add_coef(mat, constraint, variable, matrix_coef[constraint-1][variable-1]);
}

void add_constraint(glp_prob *lp, struct matrix *mat, int constraint, double upper_bound)
{
